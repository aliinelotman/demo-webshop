...
export class CarouselSettingsComponent implements OnInit {
  ...
  onSubmit(form: NgForm) {
    ...
    this.carouselPictureService
      .addCarouselPicture(newCarouselPicture)
      .subscribe((res) => {
        this.images = res;
      });
  }

  onDeleteImg(carouselPicture: CarouselPicture) {
    // const index = this.images.findIndex(element => element.imgName === image.imgName);
    // this.images.splice(index,1);
    // TODO : Delete image from backend
    //Kodune 2.04:
    let id = carouselPicture.id;
    this.carouselPictureService.deleteCarouselPicture(id).subscribe((res) => {
      console.log(res);
    });
  }
}
